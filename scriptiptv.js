...
// === YouTube Autoplay Gate (add-on autonome) ===
// Affiche un bouton "Autoriser la lecture YouTube" UNIQUEMENT si
// YouTube est sélectionné en premier et que le navigateur bloque l’autoplay.
// Ne modifie pas ta logique existante (loadSource, render, etc.).
(function(){
  ...
  // Un seul geste global suffit pour la session
  window.addEventListener('pointerdown', markUnlocked, { once:true });
  window.addEventListener('keydown',    markUnlocked, { once:true });
  ...
  function currentSrc(){
    try {
      const s = (typeof player.currentSource === 'function') ? player.currentSource() : null;
      return s && s.src ? s.src : (typeof player.currentSrc === 'function' ? player.currentSrc() : '');
    } catch(_) { return ''; }
  }
  function ensureIframeAllowAutoplay(){
    try{
      const ifr = player.el() && player.el().querySelector && player.el().querySelector('iframe');
      if (ifr) {
        const prev = ifr.getAttribute('allow') || '';
        if (!/autoplay/.test(prev)) ifr.setAttribute('allow', 'autoplay; fullscreen; picture-in-picture');
      }
    }catch(_){}
  }
  function showYouTubeGate(onApprove){
    ...
    gate.style.cssText = 'position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.35);backdrop-filter:saturate(1.05) blur(1.5px);z-index:2147483647;pointer-events:auto';
    const btn = document.createElement('button');
    btn.textContent = 'Autoriser la lecture YouTube';
    btn.style.cssText = 'padding:.65rem 1rem;border-radius:999px;border:1px solid rgba(255,255,255,.55);background:rgba(51,136,255,.92);color:#fff;font-weight:600;box-shadow:0 6px 18px rgba(0,0,0,.28);cursor:pointer';
    btn.addEventListener('click', ()=>{
      try { markUnlocked(); } catch(_){}
      try { document.body.removeChild(gate); } catch(_){ gate.classList.add('d-none'); }
  ...
  function maybeShowGate(reason){
    const src = currentSrc();
    if (!isYouTubeUrl(src) || ytGateUnlocked) return;
    ensureIframeAllowAutoplay();
    showYouTubeGate(()=> { try{ player.play(); }catch(_){ } });
  }

  // Au démarrage d’une source : si c’est YT et que ça ne part pas très vite → gate.
  player.on('loadstart', ()=>{
    const src = currentSrc();
    if (!isYouTubeUrl(src) || ytGateUnlocked) return;
    ensureIframeAllowAutoplay();
    setTimeout(()=>{ if (player.paused()) maybeShowGate('loadstart-paused'); }, 250);
  });

  // En cas d’erreur avec une source YT, propose le gate
  player.on('error', ()=> { maybeShowGate('error'); });

  // Dès que ça joue, on retire le gate s’il existe
  player.on('playing', ()=>{
    const g = document.getElementById('ytGestureGate');
    if (g) { try{ g.remove(); }catch(_){ g.classList.add('d-none'); } }
  });
})();
...
// --- Chargement automatique à la sélection d'une nouvelle playlist ---
['#sourceSelect', '#sourceSelect2'].forEach(selId => {
  ...
  sel.addEventListener('change', () => {
    ...
    currentIndex = -1;
    console.log('[Autoload] Playlist changée → chargement automatique:', val);
    loadM3UFromUrl(val);
  });
});
    ...
    $('#m3uFile').addEventListener('change', e=> { autoplayPending = true; currentIndex = -1; loadM3UFromFile(e.target); });
    $('#m3uFile2').addEventListener('change', e=> { autoplayPending = true; currentIndex = -1; loadM3UFromFile(e.target); });
...
// ===== Playlist Directory (JSON) support =====
  async function loadPlaylistDirectory(input){
    try {
      ...
      const targets = [document.getElementById('sourceSelect'), document.getElementById('sourceSelect2')].filter(Boolean);
      if (!targets.length) {
        // DOM pas prêt ? on retente une fois
        setTimeout(() => loadPlaylistDirectory(input), 300);
        return;
      }

      for (const sel of targets){
        sel.querySelector('optgroup[data-jsondir]')?.remove();
        const og = document.createElement('optgroup');
        og.setAttribute('label', '📁 JSON');
        og.setAttribute('data-jsondir','1');
